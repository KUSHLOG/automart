  ...
  const handlePlaceBid = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (highestBid && amount <= highestBid.amount) {
        setError(
          `Bid must be higher than current highest bid of LKR ${highestBid.amount.toLocaleString()}`
        )
        return
      }
      ...
      <div className="bg-gradient-to-r from-blue-900/50 to-blue-800/50 p-6 rounded-xl border border-blue-700">
        ...
        {highestBid ? (
          <div className="space-y-2">
            ...
            <p className="text-gray-400 text-sm">by {highestBid.bidder.name || 'Anonymous'}</p>
          </div>
        ) : (
